...
func (server *Server) createTranser(c *gin.Context) {
	...
	if err != nil {
		fmt.Println("error in createTranser")
...
func (server *Server) validAccount(c *gin.Context, accountID int64, currency string) bool {
	...
	if err != nil {
		...
		fmt.Println("error in validAccount")
		c.JSON(http.StatusInternalServerError, errorResponse(err))
		return false
	}

	if account.Currency != currency {
		err := fmt.Errorf("account [%d] currency mismatch: %s", accountID, account.Currency)
		c.JSON(http.StatusBadRequest, errorResponse(err))
		return false
